 ...
 */
/**
 * SNOW LYING ON A SURFACE, as a covering rather than as objects placed on it.
 *
 * The standard technique, and the one every engine uses: take the dot of the
 * surface's WORLD normal with up, threshold it, and blend the albedo toward
 * snow where the surface faces the sky. Sky-facing faces go white, vertical
 * ones stay bare, and the transition sits wherever the threshold is put. It
 * covers branches and leaves alike without a single added triangle, and it
 * follows the real geometry exactly, which is what makes it read as snow on a
 * thing rather than as white things near a thing.
 *
 * WORLD normal, not view and not object. The clusters are instanced, so their
 * local +Y points wherever the instance was turned — snow computed in object
 * space would lie on the sides of half the tree. It is built in the vertex
 * shader from objectNormal through instanceMatrix and then modelMatrix, which
 * is the only place all three are available together.
 *
 * `noise` breaks the line so it is not a clean latitude across every leaf, and
 * `lift` pushes covered vertices along world up so the snow has some thickness
 * instead of being paint.
 *
 * Composes with whatever hook the material already has — the leaves carry a
 * zero-specular hook and a sway hook, and replacing either would silently
 * un-matte the tree or stop the wind.
 */
export function applySnow(material, { amount = 1, color = 0xeef4fb, lo = 0.28, hi = 0.72, noise = 0.35, lift = 0.05, key = 'snow' } = {}) {
  ...
  // util.js is deliberately free of imports, so the colour is unpacked by hand
  // rather than through THREE.Color. `color` may be a hex number or an [r,g,b].
  ...
  material.onBeforeCompile = (shader, renderer) => {
    ...
    shader.uniforms.uSnowAmt = { value: amount };
    shader.uniforms.uSnowCol = { value: c };
    shader.uniforms.uSnowLo = { value: lo };
    shader.uniforms.uSnowHi = { value: hi };
    shader.uniforms.uSnowNoise = { value: noise };

    shader.vertexShader = 'varying float vSnowUp;\nvarying vec3 vSnowWP;\n' + shader.vertexShader;
    // After beginnormal_vertex, `objectNormal` exists and nothing has consumed it.
    shader.vertexShader = shader.vertexShader.replace(
      ...
      #endif
      vSnowUp = normalize( mat3( modelMatrix ) * snowN ).y;
      `
    );
    // World position for the noise, taken where sway has already moved the vertex
    // so the speckle travels with the leaf instead of swimming across it.
    shader.vertexShader = shader.vertexShader.replace(
      ...
      #endif
      vSnowWP = ( modelMatrix * snowWP ).xyz;
      `
    );

    shader.fragmentShader =
      'uniform float uSnowAmt;\nuniform vec3 uSnowCol;\nuniform float uSnowLo;\nuniform float uSnowHi;\n' +
      'uniform float uSnowNoise;\nvarying float vSnowUp;\nvarying vec3 vSnowWP;\n' + shader.fragmentShader;
    // After color_fragment, diffuseColor already carries the vertex colours this
    // project paints everything with — snow has to go on top of them.
    shader.fragmentShader = shader.fragmentShader.replace(
      '#include <color_fragment>',
      /* glsl */ `
      #include <color_fragment>
      float snowN3 = sin( vSnowWP.x * 7.3 ) * sin( vSnowWP.y * 6.1 ) * sin( vSnowWP.z * 8.7 );
      float snowT = smoothstep( uSnowLo, uSnowHi, vSnowUp + snowN3 * uSnowNoise );
      diffuseColor.rgb = mix( diffuseColor.rgb, uSnowCol, snowT * uSnowAmt );
      `
    );
  };
  const k = `${key}-${lo}-${hi}-${noise}`;
  material.customProgramCacheKey = priorKey ? () => `${priorKey.call(material)}+${k}` : () => k;
}
